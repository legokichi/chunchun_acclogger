...
(function() {
  ...
  main = function() {
    ...
    error_removed_matrix = transposed_matrix.filter(function(cols) {
      ...
      return movements.every(function(a) {
        return movements[0] === a && a.length !== 0;
      });
    });
    trainingVectors = error_removed_matrix.map(function(cols) {
      ...
      timestamps = cols[0], secs = cols[1], xs = cols[2], ys = cols[3], zs = cols[4], alphas = cols[5], betas = cols[6], gammas = cols[7], movements = cols[8], states = cols[9];
      return [
        movements[0], [xs, ys, zs, alphas, betas, gammas].map(function(a) {
          var conved;
          conved = a.reduce((function(lst, row, i) {
            if (i % 32 === 0) {
              lst.push([]);
            }
            lst[lst.length - 1].push(row);
          ...
          return [conved.map(average), conved.map(variance)].join(",");
        })
      ];
    });
