...
(function() {
  var KDE, average, derivative, findMax, findMin, freqToIndex, fs, indexToFreq, log, main, median, mode, stdev, summation, variance;

  fs = require('fs');

  main = function() {
    var casted_matrix, convoluted_matrix, csv, error_removed_matrix, fft, inputPath, matrix, myPath, trainingVectors, transposed_matrix, _, _ref;
    fft = (function(fft) {
      return function(arr) {
        var j, w, _arr, _i, _len, _ref;
        _arr = arr.slice();
        fft.forward(_arr);
        _ref = fft.spectrum;
        for (j = _i = 0, _len = _ref.length; _i < _len; j = ++_i) {
          w = _ref[j];
          fft.spectrum[j] *= 20 * -1 * Math.log((fft.bufferSize / 2 - j) * (0.5 / fft.bufferSize / 2)) * fft.bufferSize;
        }
        return _arr;
      };
    })(new FFT(128, 20));
    _ref = process.argv, _ = _ref[0], myPath = _ref[1], inputPath = _ref[2];
    csv = fs.readFileSync(inputPath, {
      encoding: "utf-8"
    });
    matrix = csv.split("\n").map(function(a) {
      return a.split(",");
    });
    casted_matrix = matrix.map(function(_arg) {
      var alpha, beta, gamma, movement, sec, state, timestamp, x, y, z;
      timestamp = _arg[0], sec = _arg[1], x = _arg[2], y = _arg[3], z = _arg[4], alpha = _arg[5], beta = _arg[6], gamma = _arg[7], movement = _arg[8], state = _arg[9];
      return [Number(timestamp), Number(sec), Number(x), Number(y), Number(z), Number(alpha), Number(beta), Number(gamma), (movement || "").trim(), (state || "").trim()];
    });
    convoluted_matrix = casted_matrix.reduce((function(lst, row, i) {
      if (i % 128 === 0) {
        lst.push([]);
      }
    ...
    convoluted_matrix = convoluted_matrix.filter(function(a) {
      return a.length === 128;
    });
    transposed_matrix = convoluted_matrix.map(function(_128) {
      var cols, i;
      cols = (function() {
        var _i, _ref1, _results;
        _results = [];
        for (i = _i = 1, _ref1 = _128[0].length; 1 <= _ref1 ? _i <= _ref1 : _i >= _ref1; i = 1 <= _ref1 ? ++_i : --_i) {
          _results.push([]);
        }
        return _results;
      })();
      _128.forEach(function(row) {
        return row.forEach(function(data, i) {
          return cols[i].push(data);
        });
      });
      return cols;
    });
    error_removed_matrix = transposed_matrix.filter(function(cols) {
      var alphas, betas, gammas, movements, secs, states, timestamps, xs, ys, zs;
      timestamps = cols[0], secs = cols[1], xs = cols[2], ys = cols[3], zs = cols[4], alphas = cols[5], betas = cols[6], gammas = cols[7], movements = cols[8], states = cols[9];
      return states.every(function(a) {
        return states[0] === a && a.length !== 0;
      });
    });
    trainingVectors = error_removed_matrix.map(function(cols) {
      var alphas, betas, gammas, movements, secs, states, timestamps, xs, ys, zs;
      timestamps = cols[0], secs = cols[1], xs = cols[2], ys = cols[3], zs = cols[4], alphas = cols[5], betas = cols[6], gammas = cols[7], movements = cols[8], states = cols[9];
      return [states[0], [average(xs), average(ys), average(zs), average(alphas), average(betas), average(gammas), variance(xs), variance(ys), variance(zs), variance(alphas), variance(betas), variance(gammas)].concat(fft(xs).slice(2, 8), fft(ys).slice(2, 8), fft(zs).slice(2, 8), fft(alphas).slice(2, 8), fft(betas).slice(2, 8), fft(gammas).slice(2, 8))];
    });
    return trainingVectors.forEach(function(_arg) {
      var label, vect;
      label = _arg[0], vect = _arg[1];
      return fs.appendFileSync(label + ".csv", vect.join(",") + "\n");
    });
  };
  ...
  indexToFreq = function(id, sampleRate, fftSize) {
    return (id * sampleRate) / fftSize;
  };

  freqToIndex = function(freq, sampleRate, fftSize) {
    return (freq * fftSize) / sampleRate | 0;
  };

  summation = function(ary) {
    var sum, v, _i, _len;
    sum = 0;
  ...
  variance = function(ary) {
    var ave, sum, v, _i, _len;
    ave = average(ary);
    sum = 0;
    for (_i = 0, _len = ary.length; _i < _len; _i++) {
      v = ary[_i];
  ...
  derivative = function(ary) {
    var i;
    return [0].concat((function() {
      var _i, _ref, _results;
      _results = [];
      for (i = _i = 1, _ref = ary.length - 1; 1 <= _ref ? _i <= _ref : _i >= _ref; i = 1 <= _ref ? ++_i : --_i) {
        _results.push(ary[i] - ary[i - 1]);
      }
      return _results;
    })());
  };
  ...
  KDE = function(ary, h) {
    var f, kernel, x, _i, _len, _results;
    if (h == null) {
  ...
  mode = function(ary) {
    return ary[findMax(KDE(ary))[1]];
  };

  findMax = function(ary, min, max) {
    var i, index, result, _i;
    if (min == null) {
      min = 0;
    }
    if (max == null) {
