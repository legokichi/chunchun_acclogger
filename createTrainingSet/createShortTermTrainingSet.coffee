...
main = ->
  ...
  matrix = csv.split("\n").map (a)-> a.split(",")
  casted_matrix = matrix.map ([timestamp,sec,x,y,z,alpha,beta,gamma,movement,state])->
    [
      Number(timestamp),
      Number(sec),
      Number(x),
  ...
  error_removed_matrix = transposed_matrix.filter (cols)->
    [timestamps,secs,xs,ys,zs,alphas,betas,gammas,movements,states] = cols
    movements.every (a)-> movements[0] is a and a.length isnt 0
  trainingVectors = error_removed_matrix.map (cols)->
    [timestamps,secs,xs,ys,zs,alphas,betas,gammas,movements,states] = cols
    [
      movements[0],
      [xs,ys,zs,alphas,betas,gammas].map (a)->
        conved = a.reduce(((lst, row, i)->
          if i % 32 is 0
          then lst.push []
          lst[lst.length-1].push row
          lst
