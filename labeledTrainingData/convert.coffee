...
main = ->
  fft = do (fft = new FFT(128, 20))-> (arr)->
    _arr = arr.slice()
    fft.forward(_arr)
    for w, j in fft.spectrum
      fft.spectrum[j] *= 20 * -1 * Math.log((fft.bufferSize/2 - j) * (0.5/fft.bufferSize/2)) * fft.bufferSize # equalize, attenuates low freqs and boosts highs
    _arr
  [_, myPath, inputPath] = process.argv
  csv = fs.readFileSync(inputPath, {encoding: "utf-8"})
  matrix = csv.split("\n").map (a)-> a.split(",")
  casted_matrix = matrix.map ([timestamp,sec,x,y,z,alpha,beta,gamma,state,movement])->
    [
      Number(timestamp),
      Number(sec),
      Number(x),
  ...
  convoluted_matrix = casted_matrix.reduce ((lst, row, i)->
    if i % 128 is 0
    then lst.push []
    lst[lst.length-1].push row
    lst
  ), []
  convoluted_matrix = convoluted_matrix.filter (a)-> a.length is 128
  transposed_matrix = convoluted_matrix.map (_128)->
    cols = ([] for i in [1.._128[0].length])
    _128.forEach (row)->
      #[timestamp,sec,x,y,z,alpha,beta,gamma,state,movement] = row
      row.forEach (data, i)->
        cols[i].push data
    cols
  error_removed_matrix = transposed_matrix.filter (cols)->
    [timestamps,secs,xs,ys,zs,alphas,betas,gammas,states,movements] = cols
    states.every (a)-> states[0] is a and a.length isnt 0
  trainingVectors = error_removed_matrix.map (cols)->
    [timestamps,secs,xs,ys,zs,alphas,betas,gammas,states,movements] = cols
    [
      states[0],
      #[timestamps[0],secs[0]],
      [
        ...
        fft(ys).slice(2,8),
        fft(zs).slice(2,8),
        fft(alphas).slice(2,8),
        fft(betas).slice(2,8),
        fft(gammas).slice(2,8)
      ]
    ]
  trainingVectors.forEach ([label, vect])->
    fs.appendFileSync(label+".csv", vect.join(",")+"\n")
...
indexToFreq = (id, sampleRate, fftSize)->
  (id * sampleRate) / fftSize

freqToIndex = (freq, sampleRate, fftSize)->
  (freq * fftSize) / sampleRate|0
...
derivative = (ary)->
  [0].concat((ary[i] - ary[i-1] for i in [1..ary.length-1]))

median = (ary)->
  Array.prototype.slice.call(ary, 0).sort()[ary.length/2|0]

KDE = (ary, h)-># Kernel density estimation
  h ?= 1.06*stdev(ary)*Math.pow(ary.length, -1/5)
  kernel = (x)-> Math.pow(Math.E,-Math.pow(x,2)/2)/Math.sqrt(2*Math.PI)
  f = (x)->
    s = 0
    s += kernel((x-v)/h) for v,i in ary
    s/(h*ary.length)
  (f(x) for x in ary)

mode = (ary)->
  ary[findMax(KDE(ary))[1]]

findMax = (ary, min=0, max=ary.length-1)->
  result = -Infinity; index = -1
  for i in [min..max] when ary[i] > result then result = ary[i]; index = i
  [result, index]

findMin = (ary, min=0, max=ary.length-1)->
  result = Infinity; index = -1
  for i in [min..max] when ary[i] < result then result = ary[i]; index = i
  [result, index]
